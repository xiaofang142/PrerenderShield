...
- **使用场景**：
  - 已有运行中的Web服务，想要添加渲染预热功能
  - 需要在多个服务之间共享prerender-shield的渲染预热功能
...
| 访问模式 | 启用上游代理 | 效果 | 适合场景 |
|---------|------------|------|---------|
| 直接对外访问 | 启用 | 站点直接接收外部请求，资源请求直接返回，非资源请求返回渲染预热结果并转发到上游服务 | 已有运行中的Web服务，想要添加渲染预热功能，无需修改原有服务的代码 |
| 直接对外访问 | 禁用 | 站点直接接收外部请求，资源请求直接返回，非资源请求返回渲染预热结果，不转发到其他服务 | prerender-shield作为独立的HTTP服务器，处理所有请求 |
| 作为反向代理上游 | 启用 | 站点通过nginx接收请求，资源请求直接返回，非资源请求返回渲染预热结果并转发到上游服务 | 使用nginx处理域名解析和负载均衡，prerender-shield处理渲染预热，然后转发到后端服务 |
| 作为反向代理上游 | 禁用 | 站点通过nginx接收请求，资源请求直接返回，非资源请求返回渲染预热结果，不转发到其他服务 | prerender-shield作为静态资源服务器，处理渲染预热和静态资源请求 |

## UI交互改进

### 表单布局优化
- 使用卡片布局将表单分为基本信息、站点访问配置、渲染预热配置、防火墙配置和SSL配置等模块

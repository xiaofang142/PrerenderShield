...
### 1.4 架构优化说明 (2026-01-06)

#### 1.4.1 全 Redis 驱动配置
-   **设计目标**：移除对本地配置文件的运行时依赖，支持分布式部署。
-   **实现方式**：
    -   系统启动时，尝试从 Redis 加载 `sites` 和 `system` 配置。
    -   如果 Redis 为空（首次运行），从本地 `config.yml` 同步到 Redis。
    -   后续所有配置变更（API 操作）直接写入 Redis，并通过 Redis Pub/Sub 通知各服务实例热更新。
    -   配置键名规范：`prerender:config:sites` (Hash), `prerender:config:system` (Hash)。

#### 1.4.2 单用户认证与 Session 管理
-   **设计目标**：简化单实例部署的权限管理，增强安全性。
-   **实现方式**：
    -   **首次运行 (First Run)**：系统启动检测是否有用户注册。若无，提供 `/auth/first-run` 接口供设置初始密码。
    -   **单用户模式**：系统仅允许一个 `admin` 账号。
    -   **Stateful JWT**：JWT Token 包含 `session_id`。登录时在 Redis 存储 Session（TTL控制过期）。
    -   **注销/吊销**：注销时删除 Redis 中的 Session Key，使得即便 JWT 未过期也无法通过验证（中间件检查 Redis 状态）。

#### 1.4.3 WAF 配置持久化
-   **站点配置**：WAF 规则（GeoIP、RateLimit、Black/White List）作为站点配置的一部分存储在 Redis。
-   **前端交互**：`Sites.tsx` 表单回填逻辑优化，确保从 API 获取的深层嵌套 JSON 对象能正确映射到 Ant Design Form Fields。

### 1.5 常见问题与解决方案

#### 1.5.1 静态资源在重启后丢失
- **问题描述**：在使用 `go run` 启动项目时，每次重启后上传的静态资源都会消失。
- **原因分析**：`go run` 会将可执行文件编译到临时目录（如 `/tmp` 或 `/var/folders`），默认的静态资源目录是相对于可执行文件的。每次重启都会生成新的临时目录，导致旧资源丢失。
- **解决方案**：在 `internal/config/config.go` 中增加了环境检测逻辑。如果检测到在临时目录下运行，则强制使用当前工作目录（`os.Getwd()`）作为基准路径。

#### 1.5.2 全球访问地图显示异常
- **问题描述**：虽然有访问数据（如CN有12条记录），但地图上中国区域显示为0。
- **原因分析**：
  1. 本地测试环境默认将 Localhost 映射为 "CN"，而地图组件（ECharts）使用的是全称 "China"。
  2. 前端缺乏国家代码到全称的映射机制。
- **解决方案**：
  1. 后端：在 `visit_log.go` 中将 Localhost 的默认 GeoIP 映射修改为 "China"。
  2. 前端：在 `Overview.tsx` 中增加 `countryNameMap`，将 "CN", "US" 等代码映射为对应的全称。
